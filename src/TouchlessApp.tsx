...
const TouchlessApp: React.SFC<TouchlessAppProps> = ({children, startElement, secondaryThreshold}) => {
    React.useEffect(()=>{
        ...
        if(controlled.length === 0 && elements.length > 0){ //If there is no controlled and there are elements to be controlled
            const i = startElement && startElement <= elements.length ?  startElement-1 : 0 // i is index of what is chosen from props. If it's not specified in props it start with the first element of the list
            elements[i].classList.add(styles.active); //Add classname so that it's controlled
        }
        ...
        const keyEvent = (e: KeyboardEvent)=>{
            ...
            const sThreshold =  secondaryThreshold || 10; //Threshold is how far of an element is allowed to be positioned in the secondary direction in relation to the starting point
