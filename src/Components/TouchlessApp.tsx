...
const TouchlessApp: React.SFC<TouchlessAppProps> = ({
  ...
  React.useEffect(() => {
    ...
    const keyEvent = (e: KeyboardEvent) => {
      ...
      if (
        !['ArrowUp', 'ArrowDown', 'ArrowRight', 'ArrowLeft', 'Enter'].includes(
          e.key
        )
      ) {
        return;
    ...
    if (uid) {
      ...
      socket.on('key event', ({ key }: { key: string }) => {
        ...
        if (Object.keys(customKeys).length > 0) {
          ...
          const customKey = customKeys.hasOwnProperty(customGestureValue)
            ? customKeys[customGestureValue]
            : key;
          document.dispatchEvent(
            new KeyboardEvent('keydown', { key: customKey, bubbles: true })
          );
        } else {
          document.dispatchEvent(
  ...
  return (
    <div className={`${styles['touchless-app']} touchless-app`}>{children}</div>
  );
};
